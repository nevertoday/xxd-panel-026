...
| モード | 標準キャンバス | 成果物 |
| --- | --- | --- |
| 上下二分割 | 3:4 | 上に元写真、下にヒューマニスト幾何学。高さは厳密に半分ずつ |
| 左右二分割 | 3:2 | 左に元写真、右にヒューマニスト幾何学。幅は厳密に半分ずつ |
| デザインのみ | 3:4 | 元写真は根拠として使い、完成画面には表示しない |
| 壁紙セット | 端末別 | スマートフォン、iPad、PC、子ども向けスマートウォッチ用の独立 PNG 4点 |

ユーザー指定の正確なピクセル値は標準比率より優先されます。上下二分割は全高、左右二分割は全幅が偶数である必要があります。指定寸法を黙って変更することはありません。
...
026 は標準で、ひとつの主タイトルと 2–4 組のマイクロコピーを組み込みます。見えている事実、関係の緊張、そこから無理なく読み取れる含意を言葉へ圧縮し、無関係な写真へ置き換えても成立しないかを確認します。別の写真でも使える文なら、書き直します。

完成原稿が指定された場合は一字一句を保持します。方向性や下書きの場合は、許可された範囲だけを整えます。文字を消すのは、ユーザーが明確に「文字なし」と指定した場合だけです。

コピーの言語は、人物の外見ではなく、届け先で決まります。

**対象市場・地域 > 指定された出力言語 > 方向性に使われた言語 > 依頼文の言語**
