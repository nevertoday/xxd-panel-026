---
name: xxd-panel-026
description: "Create XXD Panel 026 artwork from supplied photos in four selectable modes: photo above/humanist geometry below, photo left/humanist geometry right, transformed design alone, or a four-device wallpaper pack with either independent compositions or anchor-linked visual continuity. Supports exact 50/50 paired layouts, custom dimensions, source-aware localized copy, soft source-derived palettes, and subtle paper-relief depth. Use for the exact quiet architectural/editorial 026 style; never use it for collages or generic pastel abstraction."
---
...
- Editorial copy is enabled by default in every mode, including all four wallpaper outputs: every artwork contains one main title and 2–4 microtext groups inside the transformed design region. Omit copy only when the user explicitly requests a text-free result.
...
scripts/compose_panel.py --source photo.png --design design.png \
    --out ~/Desktop/xxd-panel-026/IMG_4821-top-bottom/IMG_4821.png --layout top-bottom --canvas 3:4

# left-right: exact vertical seam, exact custom resolution
scripts/compose_panel.py --plan --layout left-right --size 2560x1440
scripts/compose_panel.py --source photo.png --design design.png \
    --out ~/Desktop/xxd-panel-026/IMG_4821-left-right/IMG_4821-lr.png --layout left-right --size 2560x1440

# design-only: no visible photo and no seam
scripts/compose_panel.py --design design.png \
...
**Transformed geometric design.** Generate it alone, filling the exact design frame reported by `--plan`, with no photograph and no unused placeholder space. Append this resolved mode block:
...
- Unless the user explicitly requested a text-free result, the transformed design contains the locked main title and all 2–4 locked microtext strings, accurately readable, with no substitution, misspelling, gibberish, extra wording, or invented year. The main title is visually unmistakable and at least three times the microtext scale; it is not reduced to a hairline label. For text-free mode, no letters, numbers, captions, or pseudo-text appear anywhere.
