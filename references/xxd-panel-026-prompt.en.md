...
Mode and device dimensions may change placement and aspect ratio, never the 026 aesthetic motive. Every transformed frame must visibly express: **this exact photographed subject or inseparable relationship → at least three source-specific identity cues → quiet reduction into simple geometry, fine contour or structural lines, and generous negative space → a source-derived low-stimulation humanist palette → balanced rhythm with slight offsets and restrained positive/negative shape → barely raised or pressed-paper depth → sparse editorial typography bound to the image's meaning**.

If an unrelated photo could replace the source without materially changing the subject construction, spatial relationship, palette, line logic, or copy, the result is generic and must fail. Do not replace a specific person, animal, vehicle, building, object, gesture, or relationship with generic Bauhaus circles, pastel blobs, architecture-diagram decoration, sun-and-wave symbols, stock vector scenery, or soft-coloured wallpaper. Device safe areas may move elements but must not erase source identity, quiet spatial hierarchy, subtle relief, main title, or editorial typography.
...
2. **Design evidence for the transformed frame.** Extract the most recognizable subject, outer contour, pose, orientation, proportions, center of gravity, depth relationships, structural axes, and narrative action, then translate them into quiet humanist geometry with subtle paper relief.
...
4. Rebuild the subject with simple geometry, flat restrained planes, fine contour or structural lines, and negative space. Make it correspond immediately to the original, then establish human rhythm through density, positive/negative shape, and slight offsets.
5. Translate the photograph's most atmospheric colors into four to six warm, low-stimulation roles. Add only a whisper of pressed or gently raised paper depth, never obvious 3D.
6. Create a restrained editorial hierarchy for title and microcopy, then check fixed proportions, cross-panel identity, color provenance, language, and every exclusion.

This is not tracing, automatic vectorization, a filter, a complete realistic illustration, or unrelated abstraction. Preserve the minimum clear identity cues and compress real relationships into geometry with warmth, culture, and breathing room.
...
## 4. Transformed design: quiet humanist geometry

Reconstruct the same subject and relationship as quiet, modern, warm, restrained, and recognizable humanist geometry.
...
- Keep the composition broadly centred without rigid symmetry. The subject may be gently offset, cropped, layered, or allowed to leave the transformed-frame edge, but it must retain its identity.

Adapt to content:

- **People:** preserve head-to-body proportion, posture, direction, the broad clothing silhouette, and the key action; reduce them through gentle planes and fine lines, retaining facial or hand detail only when indispensable.
...
- **Architecture:** retain one to three cues such as skyline contour, taper, arch, spire, eave line, or layered rhythm, using structural lines to organise space.
...
- **Natural landscapes:** compress them into terrain blocks, horizontal axes, gentle curves, and scale relationships rather than realistic scenery.

### 4.2 Graphic construction

- Primary vocabulary: simple silhouettes, flat restrained planes, fine contour lines, horizontal axes, spatial dividers, structural lines, and gentle curves.
- Emphasize balance, rhythm, visual centre, depth hierarchy, positive/negative shape, measured density, and slight layer offsets. Negative space is active structure, not leftover area.
- A few circles, rectangles, arcs, contour lines, or structural axes may establish space, but each must answer a real source relationship.
- Keep one visual core. The construction may feel centred, but proportion, spacing, and subtle asymmetry must prevent stiffness.
- Allow only shallow pressed-paper, barely raised relief, or a restrained edge shadow. Depth must remain tactile and implicit, never thick, glossy, card-like, or CG.
- Keep edges clean and gentle rather than aggressively industrial. Exclude obvious 3D, realistic reflections, lens glow, glassmorphism, floating cards, cel animation, cold tech UI, e-commerce styling, and generic vector templates.

## 5. Color system

Extract the photograph's most atmospheric and recognizable colors and translate them into a restrained **four-to-six-color low-stimulation humanist palette**. Do not copy pixels mechanically or apply a fixed muted-color filter.

The roles normally include:

- a warm white, ivory, pale grey, or sand ground;
- one source-derived principal color carrying the subject's mood;
- one or two misted supporting colors;
- one restrained dark structural color;
- optionally one small, gentle contrast accent.

Translation may move toward ivory, warm white, pale grey, sand, dusty pink, pale ochre, mist blue, sage, or neighboring hues, but every color must be explainable through the source's principal color, supporting color, light, or atmosphere. Build hierarchy through area, overlap, negative space, and subtle value contrast rather than complex gradients.

Avoid muddy distortion, too many competing colors, unsupported complements, neon or fluorescent hues, candy-cartoon palettes, cold tech blue, rainbow gradients, ordinary commercial palettes, cheap retro filters, and equal use of every color.

## 6. Humanist editorial typography
...
- Create clear hierarchy through large/small contrast, weight, tracking, and a quiet grid.
- Let type follow a subject contour, structural line, negative-space edge, or image axis. It may align, weave, wrap, stack vertically, or rotate 90 degrees, but gestures should remain few and exact.
...
- Typography must participate in visual weight and spatial rhythm rather than sit inside a separate commercial title box. Image and wording should feel inseparable, never like a label added afterward.
...
1. **INDEPENDENT:** every device receives only the source photograph, the full 026 aesthetic prompt, and the same locked source facts, four-to-six-color humanist palette, and copy package. No generated wallpaper becomes a reference, so the four compositions may explore more freely.
2. **LINKED:** generate the iPad wallpaper first by default (honor another anchor device when the user explicitly names one), then approve its source identity, aesthetic motive, copy, typography, and safe area before continuing. Every remaining device receives both the original photograph and that same approved anchor: the photo locks content and identity, while the anchor locks only soft palette, geometric reduction, fine-line grammar, negative-space rhythm, subtle paper relief, and typography. All three derivatives point directly to the same anchor; never chain iPad → phone → desktop → watch, because sequential references accumulate drift.
...
- **Watch:** keep the subject and a simplified but visible type hierarchy recognizable at thumbnail size while reserving the major clock/complication area. Type may be enlarged, shortened, or rearranged but not deleted by default. Preserve 026 identity with fewer, larger warm shapes, clear contour evidence, quiet space, and restrained relief rather than generic symbols or extra detail.
...
- The transformed region always retains premium, modern, quiet, human, artistic, restrained, and gentle geometry with editorial design; photography is visible only in paired modes.
