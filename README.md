...
| 模式 | 尺寸逻辑 | 成品 |
| --- | --- | --- |
| 上下双联 | 源图自适应 | 原照片在上，人文几何设计在下，每格保留完整原图尺寸，严格等高 |
| 左右双联 | 源图自适应 | 原照片在左，人文几何设计在右，每格保留完整原图尺寸，严格等宽 |
| 纯设计版 | 源图自适应 | 原照片只作为依据，不出现在最终画面，沿用源图比例与尺寸 |
| 四端壁纸套装 | 按设备 | 手机、iPad、电脑、儿童手表四张独立 PNG |

用户给出的精确像素优先；否则前三种模式按源图自适应，不再套固定比例。上下双联要求用户指定的总高度为偶数，左右双联要求用户指定的总宽度为偶数；Skill 不会偷偷修改精确尺寸。

壁纸套装也不静默套尺寸：用户需要选择常用设备预设（手机 `1440×3200`、iPad `2048×2732`、电脑 `3840×2160`、儿童手表 `1024×1024`），或逐设备输入自定义分辨率。
